...
- 감성 추출 부분                                           
  ...
  - 위와 같은 화면을 약 1분간 바라보게 하는 방식으로 피험자들의 NEUTRAL한 상태를 측정   <br>                            
  - HAPPY, SAD, ANGRY, CALM, 이 각각의 감성을 유발할 수 있는 영상을 1분동안 피험자가 시청하게 하여 이 때의 심박데이터 측정
    (*각 영상 사이에 위 그림화면을 바라보게 하여 앞서 시청한 영상이 다음 영상에서의 감성 유발에 영향을 끼치지 않도록 함*)   
  - 위 활용이론과 실험과정을 바탕으로 PPI와 Amplitude의 증감에 따라 감성을 분류함
    (*실험 결과 CALM은 유의미한 값이 나오지 않아 어플 사용 시 측정 감성에서 제외함*)
